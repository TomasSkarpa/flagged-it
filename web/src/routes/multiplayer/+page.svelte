<script lang="ts">
	...
	import { RoomSetup, RoomLobby } from '$lib/components/multiplayer';
	import { createRoom, getRoomByCode } from '$lib/api/multiplayer';
	import { currentRoom, currentPlayerId, resetMultiplayerState, onMessage } from '$lib/stores/multiplayer';
	import type { RoomConfig } from '$lib/stores/multiplayer';
	import LoadingSpinner from '$lib/components/ui/LoadingSpinner.svelte';
	import { t } from '$lib/translations';
	import { locale } from '$lib/stores/locale';

	type View = 'setup' | 'lobby' | 'join';
	...
	function handleLobbyStart() {
		...
		// This will be handled by the game page itself
		view = 'setup';
	}
	...
	{:else if view === 'join'}
		<div class="card-game max-w-md mx-auto">
			...
			<form on:submit|preventDefault={handleJoinRoom} class="space-y-4">
				<div>
					<label for="multiplayer-player-name" class="block text-sm font-medium text-text-muted mb-2">
						Your Name
					</label>
					<input
				...
				<div>
					<label for="multiplayer-room-code" class="block text-sm font-medium text-text-muted mb-2">
						Room Code
					</label>
					<input
				...
				{#if roomRequiresPassword || roomPassword}
					<div>
						<label for="multiplayer-room-password" class="block text-sm font-medium text-text-muted mb-2">
							Password {roomRequiresPassword ? '(Required)' : '(Optional)'}
						</label>
						<input
							id="multiplayer-room-password"
							type="password"
